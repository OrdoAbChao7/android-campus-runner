...
def _start_checkpoint(*, fingerprint: str = "a" * 64, enterprise: str = "target") -> WeComCheckpoint:
...
def test_free_run_confirmation_finalizes_run_reservation_before_clicking(tmp_path):
    calls = []
    class Device:
    ...
    assert confirm_free_run(
        ...
        reservation=reservation, route=route, start_checkpoint=_start_checkpoint(),
    ) is CampusRunState.RUNNING
    assert calls == [{"text": "自由跑", "timeout": 10.0}]


def test_free_run_rejects_changed_start_prompt_fingerprint_before_consuming_or_clicking(tmp_path):
    calls = []
    ...
    with pytest.raises(IntentValidationError, match="checkpoint"):
        confirm_free_run(
            ...
            observation=RunObservation("PHONE", "fingerprint", route_sha256(route), now),
            intent_registry=registry, reservation=reservation, route=route, start_checkpoint=_start_checkpoint(),
        )
...
def test_free_run_rejects_checkpoint_on_a_different_authorized_device_before_clicking(tmp_path):
    calls = []
    ...
    with pytest.raises(IntentValidationError, match="checkpoint device"):
        confirm_free_run(
            Device(), intent=intent,
            observation=RunObservation("OTHER", "fingerprint", route_sha256(route), now),
            intent_registry=registry, reservation=reservation, route=route, start_checkpoint=_start_checkpoint(),
        )
...
def test_free_run_rejects_duck_typed_registry_without_clicking(tmp_path):
    ...
    with pytest.raises(IntentValidationError, match="IntentUseRegistry"):
        confirm_free_run(
            ...
            intent_registry=NoOpRegistry(), reservation=reservation, route=route,
        )
